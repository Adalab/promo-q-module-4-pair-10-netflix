...
const App = () => {
  // state: user
  const [userId, setUserId] = useState(localStorage.get("userLoginId", ""));
  ...
  */
  useEffect(() => {
    if (userId !== "") {
      ...
      localStorage.set("userLoginId", userId);
    }
  }, [userId]);

  //useEffect para guardar el userId en local storage y mantener a la usuaria logueada.
